...
class ItemMenu extends React.Component{constructor(props){super(props)}
render(){return(<li><a href={this.props.name}><i className="medium material-icons">{this.props.icon}</i></a></li>)}}
class Menu extends React.Component{constructor(props){super(props)}
componentDidMount(){$(document).ready(function(){$(".button-collapse").sideNav()
$('.scrollspy').scrollSpy({scrollOffset:-50})
$(".logo-text").click(function(){$('html,body').animate({scrollTop:$("#home").offset().top},'slow')})})}
render(){let myItemMenu=[]
this.props.itemsMenu.forEach((item)=>{myItemMenu.push(<ItemMenu
name={item.name}
key={item.name}
icon={item.icon}/>)})
return(<div className="nav-wrapper"><nav className="hide-on-large-only"><a href="#home" className="brand-logo">Mostro Media</a><a href="#" data-activates="mobile-demo" className="button-collapse"><i className="material-icons">menu</i></a><ul className="section table-of-contents right hide-on-med-and-down">{myItemMenu}</ul></nav><ul className="section table-of-contents side-nav fixed color-menu" id="mobile-demo"><div className="logo-text"><img id="logo-mostro-media" className="center" src="img/logo_svg.svg"/><h1 className="center-align">Mostro Media</h1></div>{myItemMenu}</ul></div>)}}
class Footer extends React.Component{render(){return(<footer className="center card-panel grey darken-3"><div className="container"><ul className="social"><li className="facebook"><a href="#" className="entypo-facebook"><i className="fa fa-facebook" aria-hidden="true"></i></a></li><li className="github"><a href="https://github.com/MostroMedia" className="entypo-github"><i className="fa fa-github" aria-hidden="true"></i></a></li><li className="instagram"><a href="https://www.instagram.com/mostromedia/?hl=en" className="entypo-instagram"><i className="fa fa-instagram" aria-hidden="true"></i></a></li><li className="behance"><a href="#" className="entypo-behance"><i className="fa fa-behance" aria-hidden="true"></i></a></li><li className="linked-in"><a href="#" className="entypo-linkedin"><i className="fa fa-linkedin" aria-hidden="true"></i></a></li></ul></div><div className="container"><p>Copyright(c)2017 MOSTRO MEDIA All Rights Reserved.</p></div></footer>)}}
...
showInvader(){let space_invader=this._reactInternalInstance._renderedComponent._hostNode.lastChild
space_invader.classList.remove("scale-out");space_invader.classList.add("scale-in")}
hideInvader(){let space_invader=this._reactInternalInstance._renderedComponent._hostNode.lastChild
space_invader.classList.remove("scale-in");space_invader.classList.add("scale-out")}
render(){return(<li onMouseEnter={this.showInvader}onMouseLeave={this.hideInvader}><div draggable="true" className="center"><img className="esponsive-img" src={this.props.img}alt={this.props.alt}/><h3>{this.props.name}</h3><hr/><h4 style={{color:this.props.color}}>{this.props.quote}</h4></div><div className="contSpace scale-transition scale-out"><div className="center space-invader "></div><div className="card-panel"><span className="white-text">{this.props.position}</span></div></div></li>)}}
class Team extends React.Component{componentDidMount(){$('.listMostro').slick({centerMode:!0,centerPadding:'60px',slidesToShow:5,autoplay:!1,cssEase:'ease',responsive:[{breakpoint:800,settings:{arrows:!1,centerMode:!0,centerPadding:'40px',slidesToShow:3}},{breakpoint:400,settings:{arrows:!1,centerMode:!0,centerPadding:'0px',slidesToShow:1}}]})}
render(){let mostroTeam=[]
this.props.mostros.forEach((mostro)=>{mostroTeam.push(<ItemTeam
name={mostro.name}
key={mostro.name}
img={mostro.img}
alt={mostro.alt}
...
quote={mostro.quote}
color={mostro.color}/>)})
return(<section id="team" className="section scrollspy"><div className="container"><div className="row"><TitleContact
titulo={TitleTeam}/></div></div><ul className="listMostro">{mostroTeam}</ul><div id="borde"></div></section>)}}
...
class ItemWork extends React.Component{constructor(props){super(props)}
render(){return(<li className="card center col s12 m5 grey darken-4 text-white"><div className="card-image waves-effect waves-block waves-light"><img className="activator" src={this.props.img}alt={this.props.alt}/></div><div className="card-content"><h3 className="card-title activator">{this.props.name}<i className="material-icons right">more_vert</i></h3></div><div className="card-reveal grey darken-4 text-white"><h3 className="card-title"><i className="material-icons right">close</i>{this.props.rta}</h3><br/><p>{this.props.textoRta}</p></div></li>)}}
class PairItemsWork extends React.Component{render(){let weWork=[]
...
return(<ul id="individual-services" className="row">{weWork}</ul>)}}
class Work extends React.Component{render(){return(<section id="work" className="section scrollspy"><div className="container"><div className="row"><div id="serv-section" className="col s12 l6"><TitleContact
titulo="Consolidamos tu presencia en internet a traves de nuestros servicios de Páginas web, Aplicaciones móviles, e-learning y Motion Design."/></div><div className="col s12 l6"><PairItemsWork whatwelove={WHATWELOVE}/></div></div></div></section>)}}
...
function Init(padre){scene=new THREE.Scene()
...
return renderer}
function onResize(){width=window.innerWidth
...
render()}
function render(){particles.rotation.x+=0.0008
particles.rotation.y-=0.0009
renderer.render(scene,camera)}
function drawParticles(){particles=new THREE.Group()
scene.add(particles)
const geometry=new THREE.TetrahedronGeometry(5,0)
for(let i=0;i<200;i++){const material=new THREE.MeshPhongMaterial({color:colores[Math.floor(Math.random()*colores.length)],shading:THREE.FlatShading})
const mesh=new THREE.Mesh(geometry,material)
mesh.position.set((Math.random()-0.5)*1000,(Math.random()-0.5)*1000,(Math.random()-0.5)*1000)
mesh.updateMatrix()
mesh.matrixAutoUpdate=!1
particles.add(mesh)}}
class ParticlesHome extends React.Component{constructor(){super()}
componentDidMount(){Init(document.getElementById("particles-home"))
animate()}
render(){return(<div id="particles-home"></div>)}}
...
let anim=''
let isThrowing=!1
class HomeContent extends React.Component{constructor(props){super(props)
this.playAnimation=this.playAnimation.bind(this)
this.stopAnimation=this.stopAnimation.bind(this)}
componentDidMount(){animData.container=document.getElementById('logo-mostro-media-center')
anim=bodymovin.loadAnimation(animData)
anim.playSegments([0,35],!0)
setTimeout(()=>{anim.stop()},2200)}
playAnimation(){anim.playSegments([0,35],!0)}
stopAnimation(){anim.stop()}
render(){return(<div id="mostro-center" className="container"><div className="row"><div className="col s12 m12 l12"><div id="logo-mostro-media-center" className="center" onMouseEnter={this.playAnimation}onMouseLeave={this.stopAnimation}></div><h1 className="center-align">MOSTRO<br/>MEDIA</h1></div></div></div>)}}
class SingleIconHome extends React.Component{constructor(props){super(props)}
componentDidMount(){let miAnima=this.props.anima
...
render(){return(<li><div id={this.props.myid}></div><p>{this.props.name}</p></li>)}}
class IconHome extends React.Component{componentDidMount(){setTimeout(()=>{document.getElementById("iconos-home").style.transform="scale(1)"},1000)}
render(){let myIcon=[]
this.props.iconos.forEach((icono,index,array)=>{myIcon.push(<SingleIconHome
name={icono.name}
key={icono.name}
myid={icono.name}
img={icono.img}
...
componentDidMount(){Materialize.toast('Bienvenid@',3000,'rounded')
Materialize.toast('Haz scroll y mira nuestro contenido, nuestros servicios y nuestro equipo.',5000,'rounded')}
render(){return(<section id="home" className="container-fluid section scrollspy"><HomeContent/><IconHome iconos={ICONOS}/></section>)}}
class Main extends React.Component{constructor(props){super(props)
...
ingresaNombre(event){event.preventDefault()
this.setState({value:event.target.value})}
botonPresionado(event){this.setState({isPressed:!0})}
render(){return(<div><Home/><Work/><Team mostros={MOSTROTEAM}/><Contact textContact={TextoContact}/><Footer/><ParticlesHome/><Menu itemsMenu={MiMenu}/></div>)}}
